...
def load_selected_class(prefix=""):
    """
    分层次选择文件夹、文件和类。
    prefix: 用于生成唯一的 key，避免 Streamlit 元素 ID 冲突。
    """
    # 基础路径
    base_path = os.path.join("models", st.session_state.get('selected_category', ''),
    ...
    selected_folder = st.selectbox("选择文件夹", folders, key=f"{prefix}_select_folder")
    ...
    selected_file = st.selectbox("选择文件", py_files, key=f"{prefix}_select_file")
    ...
    selected_class = st.selectbox("选择类", classes, key=f"{prefix}_select_class")
