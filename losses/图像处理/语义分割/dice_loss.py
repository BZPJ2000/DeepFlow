...
class ExpLog_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        # with torch.no_grad():
        #     print('dc losses:', dc_loss.cpu().numpy(), 'ce losses:', ce_loss.cpu().numpy())
        #     a = torch.pow(-torch.log(torch.clamp(dc_loss, 1e-6)), self.gamma)
        #     b = torch.pow(-torch.log(torch.clamp(ce_loss, 1e-6)), self.gamma)
        #     print('ExpLog dc losses:', a.cpu().numpy(), 'ExpLogce losses:', b.cpu().numpy())
        #     print('*'*20)
