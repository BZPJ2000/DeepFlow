...
class NodeClassificationDataset(Dataset):
    def __init__(self, root, transform=None, pre_transform=None):
        ...
        """
        super(NodeClassificationDataset, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            y = data['node_labels']  # 节点标签 (num_nodes,)
...
if __name__ == "__main__":
    # 数据集根目录
    root = "E:/Github_Project/troch_GUI/dataset/图像处理/节点分类"

    # 创建数据集实例
    dataset = NodeClassificationDataset(root)

    # 获取第一个样本
    sample = dataset[0]
    print("Node features:", sample.x)
    print("Edge index:", sample.edge_index)
    print("Node labels:", sample.y)
    ...
    for batch in dataloader:
        ...
        print("Batch node labels:", batch.y)
