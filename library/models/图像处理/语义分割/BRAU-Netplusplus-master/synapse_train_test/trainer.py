...
def trainer_synapse(args, model, snapshot_path):
    os.makedirs(os.path.join(snapshot_path, 'maskovrd'), exist_ok=True)
    test_save_path = os.path.join(snapshot_path, 'maskovrd')
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            logging.info('iteration %d : losses : %f, loss_ce: %f, loss_dice: %f' % (iter_num, loss.item(), loss_ce.item(), loss_dice.item()))
            if iter_num % 20 == 0:
